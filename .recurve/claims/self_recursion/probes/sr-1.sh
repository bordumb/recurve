...
# ITSELF, instead of erroring "no workflow — run init first". This is the
# plumbing that lets the loop run on its own repo, wherever its config lives
# (root or contained). RED-first: a resolver that cannot find a workflow on the
# self-host repo is RED.
