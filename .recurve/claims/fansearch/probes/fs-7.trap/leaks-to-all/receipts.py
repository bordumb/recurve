...
from recurvelib.core.conformance import Matrix
from recurvelib.io.records import RecordError, make_receipt, receipt_hash, validate_receipt
...
def discovery_provenance(config: Config) -> dict[str, dict]:
    """Gap id -> {domain, proxy_score} for claims authored by an automated
    search rather than by hand. Reads `<state_dir>/fansearch/promotions.jsonl`
    (one JSON object per line, each with a `gap` key) if present; the file
    is written once, at promotion time, never by the receipt writer."""
    path = config.state_dir / "fansearch" / "promotions.jsonl"
    if not path.exists():
        return {}
    out: dict[str, dict] = {}
    for line in path.read_text().splitlines():
        ...
        if gap_id:
            out[gap_id] = {"domain": entry["domain"], "proxy_score": entry["proxy_score"]}
    return out
...
def emit_for_matrix(config: Config, matrix: Matrix) -> int:
    ...
    discovered = discovery_provenance(config)
    count = 0
