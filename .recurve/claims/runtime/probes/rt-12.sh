#!/usr/bin/env bash
# RT-12: STOP_REVERT restores the recorded last-green snapshot, not the current world state (A1).
...
try:
    ...
    # cycle 0 is the green floor; then the world thrashes RED for three cycles until it reverts.
...
if v is Verdict.STOP_REVERT and w.restored_to == 0:
    print("revert restores the cycle-0 green floor, not the current (cycle 3) state")
    sys.exit(0)
print(f"ours=(verdict={v}, restored_to={w.restored_to}) oracle=(STOP_REVERT, 0) "
      f"(restoring the current checkpoint leaves the damage in place)")
sys.exit(1)
PYEOF
