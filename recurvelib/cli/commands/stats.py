...
def cmd_stats(args):
    ...
    # waived guard is a closed claim the drill cannot audit. Ledger-derived —
    # available regardless of whether any run record exists yet.
    ledger_closed = 0
    try:
        ledger = _load(cfg)
        ledger_closed = sum(1 for g in ledger.gaps if g.status is Status.CLOSED)
        waived = sum(1 for g in ledger.gaps if g.status is Status.CLOSED and g.trap_waiver)
    ...
    # combined, sliceable by phase. 0/N on a ledger with no challenges. This
    # is a LEDGER-derived rate (closed claims vs. recorded challenges), not a
    # run-record one — it must surface even before the first cycle record
    # ever lands (drill --diff can find a disagreement on day one).
    try:
        ...
        rate = (len(events) / ledger_closed) if ledger_closed else 0.0
        print(render.dim(
            f"challenge rate: {len(events)}/{ledger_closed} ({rate:.1%}) — "
