...
# Environment variables that address a DIFFERENT, outer probe invocation
# (e.g. this very governor being re-executed under `recurve drill`/a trap
# audit) and must never leak into the re-execution this tier performs on the
# cycle's own claims — otherwise an ambient TRAP_FIXTURE meant for someone
# else's probe would silently flip this governor's re-executed verdicts.
_AMBIENT_PROBE_ENV = ("TRAP_FIXTURE", "ISO_FIXTURE", "RECURVE_PROBE")


@contextmanager
def _without_ambient_probe_env():
    saved = {k: os.environ.pop(k) for k in _AMBIENT_PROBE_ENV if k in os.environ}
    try:
        yield
    finally:
...
class MechanicalGovernor:
    def audit(self, cycle) -> GovernorVerdict:
        ...
        vetoes: dict[str, str] = {}
        with _without_ambient_probe_env():
            for cid in cycle.claim_ids:
