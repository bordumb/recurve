...
**{{PROJECT}}**. Your first action: `{{PROG}} review <ID>` for the brief.
...
no probe tests yet. That is why a green `{{PROG}} matrix --gate` is necessary
...
- `{{PROG}} matrix --gate` is green fleet-wide.
...
- The decision is recorded in three synchronized places via
  `{{PROG}} adjudicate <ID>`: the ledger's `smallest_fix` (DECIDED <date>),
