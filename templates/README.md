...
```bash
{{PROG}} ledger      # every claim and its status — the red backlog is the honest one
{{PROG}} matrix      # run every probe: GREEN/RED/BROKEN/STALE, and the gate verdict
{{PROG}} next        # the highest-value gap to work on right now
```

## How this works (60 seconds)

Every promise this project makes exists in three synchronized places: prose
(`claims/<{{LABEL}}>/GAPS.md`), a ledger entry (`gaps.yaml`), and an
...
1. Write the prose section in .recurve/claims/<{{LABEL}}>/GAPS.md (observable + negative space).
...
4. `{{PROG}} baseline <{{LABEL}}>` — the ceremony measures it for real and
