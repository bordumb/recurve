...
description: Run the adversarial review protocol on a review-gated (security-tradeoff) gap for {{PROJECT}} — try to BREAK the change, never to confirm it
---
...
1. `{{PROG}} review <ID>` prints the brief: what is now accepted, which
   attacks to attempt.
...
4. Promotion is the human's call via `{{PROG}} adjudicate <ID>` — you report,
   you do not promote.
